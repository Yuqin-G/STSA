...
class Finetune(BaseLearner):
    ...
    def reset(self):
        if ("vit" in self.args['net']):
            self._network = SimpleVitNet(self.args, True)
        else:
    ...
    def incremental_train(self, data_manager):
        ...
        print("========") 
        print(test_acc)
        # torch.save(copy.deepcopy(self._network.state_dict()), './check/finetune1/{}_{}_{}.pkl'
                    # .format(self.args["dataset"], self.args["net"], self._cur_task)) 
        torch.save(copy.deepcopy(self._network), './check/finetune1/{}_{}_{}.pkl'
                        .format(self.args["dataset"], self.args["net"], self._cur_task)) 
    ...
    def _local_update(self, model, train_data_loader, test_loader):
        model.train()
    ...
    def _fl_train(self, train_dataset, test_loader):
        ...
        for _, com in enumerate(range(self.args["com_round"])):
            ...
            for idx in idxs_users:
                ...
                # if self._cur_task == 0:
                w = self._local_update(copy.deepcopy(self._network), local_train_loader, test_loader)
                # else:
