...
class NAYER():
    ...
    def student_train(self, student, teacher):
        ...
        for _, com in enumerate(range(student_train_step)):
...
class LANDER(BaseLearner):
    def __init__(self, args):
        ...
        with open(le_name, "rb") as label_file:
            ...
            label_emb = label_emb.to(args["gpu"])
    ...
    def data_generation(self):
        ...
        print("For task {}, start data generation! ".format(self._cur_task))
    ...
    def get_syn_data_loader(self):
        if self.args["dataset"] == "cifar100":
            ...
            dataset_size = 130000
        elif self.args["dataset"] == "imagenetr":
            dataset_size = 24000
    ...
    def incremental_train(self, data_manager):
        ...
        self._network.update_fc1(self._total_classes)
        self._network.cuda()
        ...
        if self.args['type'] == -1 or (self.args['type'] > -1 and self._cur_task >= self.args['type']) or self.args['syn']:
            torch.save(copy.deepcopy(self._network.state_dict()), './check/lander1/global_{}_{}_{}_beta{}_{}_{}_{}_{}.pkl'
                        .format(self.args["dataset"], self.args["net"], self.args["num_users"],
    ...
    def _fl_train(self, train_dataset, test_loader):
        ...
        if self.args["dataset"] == "tiny_imagenet" or "imagenet" in self.args["dataset"] :
        ...
        for epoch, com in enumerate(range(self.args["com_round"])):
            # if(epoch % 50 == 0): 
            #     print('\n')
            #     print("====================================================")
            #     print(epoch) 
            ...
            if com % 1 == 0 and com < self.args["com_round"] and self._cur_task == 0:
                ...
                if self.wandb == 1:
                    wandb.log({'Task_{}, accuracy'.format(self._cur_task): test_acc})
            elif (com + 1) % 25 == 0 and com < self.args["com_round"]:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = ("Task {}, Epoch {}/{} =>  Test_accy {:.2f}".format(
                    self._cur_task, com + 1, self.args["com_round"], test_acc, ))
                # prog_bar.set_description(info)
                ...
                if self.wandb == 1:
                    wandb.log({'Task_{}, accuracy'.format(self._cur_task): test_acc})
        # torch.save(self.best_model, './best_model.pkl')
        # exit(0)
