...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iImageNet1000, TinyImageNet200, iCIFAR224, iImageNetR, iImageNetA
import torch, copy
...
def partition_data(y_train, beta=0.4, n_parties=5, shot=None, eta=0.6):
    if shot is not None:  # 当指定 shot 时
        num_classes = len(np.unique(y_train))  # 类别总数
        # print(y_train)
        unq, _ = np.unique(y_train, return_counts=True)
        # print(unq)
        # print(num_classes)
        # print(int(eta * num_classes))
        net_dataidx_map = {i: [] for i in range(n_parties)}  # 初始化客户端数据映射
        for party in range(n_parties):
            # 随机选择每个客户端分配的类
            current_class = random.sample(
                list(unq),
                int(eta * num_classes)
            )
            # print(current_class)
            for cls in current_class:
                idx_k = np.where(y_train == cls)[0]  # 找到属于该类的样本索引
                # print(idx_k)
                np.random.shuffle(idx_k)  # 打乱样本
                net_dataidx_map[party].extend(idx_k)  # 分配样本给当前客户端
...
class DataManager(object):
    ...
    def get_dataset(
        self, indices, source, mode, appendent=None, ret_data=False, m_rate=None, shot=None
    ):
        if source == "train":
