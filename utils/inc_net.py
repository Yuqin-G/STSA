...
def get_convnet(args, pretrained=False):
    ...
    if name == "resnet32":
        ...
        checkpoint_model.out_dim=768
        for name, p in checkpoint_model.named_parameters():
            p.requires_grad = False 
        return checkpoint_model.eval()
    elif name == "vit_adapter":
...
class SimpleVitNet(BaseNet):
    ...
    def update_fc(self, nb_classes, nextperiod_initialization=None, type=0):
        ...
        if self.fc is not None:
            # fc = CosineLinear(feature_dim, nb_classes).cuda()
            ...
            if nextperiod_initialization is not None:
                ...
                weight = torch.cat([weight, torch.zeros(nb_classes - nb_output, feature_dim).cuda()])
...
class IncrementalNet(BaseNet):
    ...
    def update_fc1(self, nb_classes):
